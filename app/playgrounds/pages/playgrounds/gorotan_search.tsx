...
const GorotanSearchMain = () => {
  const gorotanWords = GOROTAN_WORDS
  ...
  return (
    <>
      <Head>
        ...
        <meta property="og:url" content="https://razokulover.com/playgrounds/gorotan_search" />
      ...
      >
        <Flex w="100%" flexDir={"column"} alignItems={"center"} my={20}>
          ...
          <Input type={"text"} placeholder={"単語を入力してください"} onChange={onChange} mb={4} />
              ...
              return (
                <>
                  ...
                  >
                    ...
                    <Flex w={"100%"} flexDir={"column"} justifyContent={"start"}>
                      {word.images.map((image) => {
                        return (
                          <Box
                            key={`${word.word}${Math.random().toString(36).substr(2, 9)}`}
...
const GorotanSearchPage: BlitzPage = () => {
  return <GorotanSearchMain></GorotanSearchMain>
}

GorotanSearchPage.authenticate = false
GorotanSearchPage.getLayout = (page) => <Layout>{page}</Layout>
